...
export default function Skills({ scrollTo }) {
  return (
    <section id="skills" className="h-screen flex flex-col justify-center items-center bg-theme-background2 text-theme-words px-40">
      <Wrapper animation={fadeScale}>
        <h1 className="text-4xl font-bold text-center m-10">skills</h1>
      </Wrapper>
      <div className={styles.skillsContainer}>
        <motion.div variants={containerStagger} initial="initial" whileInView="whileInView" viewport={{ once: false, amount: 0.2 }}>
          {skills.map((skill, index) => {
            ...
            return (
              <motion.div key={index} variants={childFadeUp} className="relative group flex flex-col items-center">
                ...
                <span className="absolute bottom-0 translate-y-8 opacity-0 text-theme-words2 bg-theme-background rounded-md px-2 py-1 transition-all duration-300 group-hover:translate-y-2 group-hover:opacity-100">{label}</span>
              </motion.div>
            );
          })}
        </motion.div>
